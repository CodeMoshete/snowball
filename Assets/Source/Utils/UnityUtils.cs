...
namespace Utils
{
    public static class UnityUtils
    {
        ...
        public static List<GameObject> GetTopLevelChildren(GameObject parent)
        {
            List<GameObject> results = new List<GameObject>();
            Transform transform = parent.transform;
            for (int i = 0, count = transform.childCount; i < count; i++)
        ...
        public static List<Transform> GetTopLevelChildTransforms(GameObject parent)
        {
        ...
        private static void GetAllChildTransforms(Transform parent, ref List<Transform> results)
        {
            Transform transform = parent.transform;
            results.Add(parent);
            ...
            for (int i = 0, count = transform.childCount; i < count; i++)
            {
                GetAllChildTransforms(transform.GetChild(i), ref results);
            }
        }

        public static List<Transform> GetAllChildTransforms(GameObject parent)
        {
            List<Transform> results = new List<Transform>();
            GetAllChildTransforms(parent.transform, ref results);
        ...
        public static List<GameObject> FindAllGameObjectContains(string name, string excludes = "", bool exact = false)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                List<GameObject> allObjects = 
                    new List<GameObject>(Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None));
